...
import { FiUsers, FiMapPin, FiEdit2, FiTrash2, FiX, FiAlertTriangle } from 'react-icons/fi';
...
const Overview = () => {
  ...
  const filteredCenters = assignedCenters?.filter(center => {
    const centerNameMatch = center.name?.toLowerCase().includes(filters.centerName.toLowerCase());
    const areaMatch = center.area?.toLowerCase().includes(filters.area.toLowerCase());
    const sadarNameMatch = center.sadarName?.toLowerCase().includes(filters.sadarName.toLowerCase());
    const tutorNameMatch = filters.tutorName
      ? (center.tutors || []).some(tutor => tutor.name?.toLowerCase().includes(filters.tutorName.toLowerCase()))
      : true;
    const statusMatch = filters.status === 'all' ? true : center.status === filters.status;
    return centerNameMatch && areaMatch && sadarNameMatch && tutorNameMatch && statusMatch;
  }) || [];
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      // await api.delete(`/centers/${deletingCenter._id}`);
