...
const Sidebar = ({ activeTab, onTabChange, className }) => {
 ...
 useEffect(() => {
    const loadUserData = () => {
      const supervisorProfile = localStorage.getItem('supervisorProfile'); // FIXED
      ...
      try {
        const parsedData = JSON.parse(supervisorProfile); // FIXED
        ...
        console.error('Error parsing supervisorProfile from localStorage:', error);
        localStorage.removeItem('supervisorProfile');
        setIsLoading(false);
        navigate('/supervisor');
      } finally {
  ...
  if (isLoading || !supervisorProfile) {
  return (
