...
const AdminManagement = () => {
  ...
  const { data: admins, loading, error: fetchError, refetch } = useGet('/supervisor');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingAdmin
        ? `${import.meta.env.VITE_API_URL}/supervisor/${editingAdmin._id}`
      ...
      setSuccessMessage(editingAdmin ? 'Supervisor updated successfully' : 'Supervisor created successfully');
  ...
  const handleDelete = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(
        `https://mtc-backend-jn5y.onrender.com/api/supervisor/${adminToDelete._id}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${JSON.parse(localStorage.getItem('userData')).token}`
  ...
  const filteredAdmins = admins?.filter(supervisor => 
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-white rounded-xl shadow overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            ...
            ) : paginatedAdmins.map((supervisor) => (
      ...
      <Popover
        ...
        title={editingAdmin ? 'Edit Supervisor' : 'Add New Supervisor'}
        type="info"
        message={
      ...
      <Popover
        ...
        message={adminToDelete ? `Are you sure you want to delete supervisor ${adminToDelete.name}?` : 'Are you sure you want to delete this supervisor?'}
        type="confirm"
