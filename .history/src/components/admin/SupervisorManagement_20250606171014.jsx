...
const SupervisorManagement = () => {
  ...
  const { data: supervisors, loading, error: fetchError, refetch } = useGet('/admin');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingSupervisor
        ? `${import.meta.env.VITE_API_URL}/admin/${editingSupervisor._id}`
  ...
  const filteredSupervisors = supervisors?.filter(admin => 
      ...
      <div className="bg-white rounded-xl shadow overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            ...
            ) : paginatedSupervisors.map((admin) => (
