                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (atIndex !== -1) {
                      ...
                      // Between @ and . (limit to 15 chars)
                      const betweenAtAndDot = dotIndex !== -1 
                        ? value.slice(atIndex + 1, dotIndex)
                        : value.slice(atIndex + 1);
                      // After . (limit to 5 chars)
                      const afterDot = dotIndex !== -1 
                        ? value.slice(dotIndex + 1)
                        : '';

                      value = `${beforeAt.slice(0, 20)}@${betweenAtAndDot.slice(0, 15)}${dotIndex !== -1 ? '.' + afterDot.slice(0, 5) : ''}`;
                    }
                    
                    // Total length limit of 40
